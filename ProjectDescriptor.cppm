...
export namespace kairo::engine
{
    ...
    struct ProjectDescriptor final
    {
        ...
        /// Optional project-relative executable launched by KairoEditor Play.
        /// When absent, hosts may fall back to the generic KairoPlayer.
        std::optional<std::filesystem::path> PlayExecutable;
    ...
    inline void ValidateProjectDescriptor(const ProjectDescriptor& descriptor)
    {
        ...
        if (descriptor.PlayExecutable.has_value())
            (void)kairo::assets::NormalizeAssetPath(*descriptor.PlayExecutable);
    ...
    [[nodiscard]] inline ProjectDescriptor ParseProjectDescriptor(std::string_view source)
    {
        ...
        while (std::getline(input, lineText))
        {
            ...
            else if (tokens[0].Text == "play-executable")
            {
                RequireCount(tokens, 2u, lineNumber, tokens[0].Text);
                if (version != 2u) throw ProjectFormatError(lineNumber,
                    tokens[0].Column, "play-executable requires project format 2");
                if (playExecutableSeen) throw ProjectFormatError(lineNumber,
                    tokens[0].Column, "duplicate play-executable statement");
                descriptor.PlayExecutable = ParsePortablePath(tokens[1], lineNumber);
                playExecutableSeen = true;
            }
    ...
    [[nodiscard]] inline std::string SerializeProjectDescriptor(const ProjectDescriptor& descriptor)
    {
        ...
        if (descriptor.PlayExecutable.has_value())
            source += "play-executable " +
                Quote(kairo::assets::NormalizeAssetPath(*descriptor.PlayExecutable).generic_string()) + "\n";
